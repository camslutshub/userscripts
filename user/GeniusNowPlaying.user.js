...
// @icon            https://raw.githubusercontent.com/TheLastZombie/userscripts/main/icons/GeniusNowPlaying.ico
...
(async function () {
  ...
  if (!username || !fmkey || !geniuskey) {
    document
      .getElementsByClassName(
        "HomeContentdesktop__CenteredFlexColumn-sc-1xfg7l1-1"
      )[0]
      .insertAdjacentHTML(
        "afterbegin",
        "<div id='gnp-config' style='padding-top: 72px; text-align: center;'><p>Thank you for installing GeniusNowPlaying! To get started, please enter the following values:</p></div>"
      );
  }

  if (!username) {
    document
      .getElementById("gnp-config")
      .insertAdjacentHTML(
        "beforeend",
  ...
  if (!username || !fmkey || !geniuskey) {
    document
      ...
      .insertAdjacentHTML(
        "beforeend",
        "<p style='margin-top: 25px;'>After saving all values, simply reload this page and you should be good to go. Enjoy!</p>"
      );
  }
  ...
  if (eUsername) {
    eUsername.onclick = async function () {
      ...
      await GM.setValue(
        "username",
        document.getElementById("gnp-input-username").value
      );
  ...
  if (eFmkey) {
    eFmkey.onclick = async function () {
      ...
      await GM.setValue(
        "fmkey",
        document.getElementById("gnp-input-fmkey").value
      );
  ...
  if (eGeniuskey) {
    eGeniuskey.onclick = async function () {
      ...
      await GM.setValue(
        "geniuskey",
        document.getElementById("gnp-input-geniuskey").value
      );
  ...
  if (username && fmkey && geniuskey) {
    GM.xmlHttpRequest({
      ...
      onload: (response) => {
        ...
        if (JSON.parse(response.responseText).recenttracks.track.length === 0)
          return;
        const fmresult = JSON.parse(response.responseText).recenttracks
          .track[0];

        GM.xmlHttpRequest({
          url:
            "https://api.genius.com/search?q=" +
            fmresult.artist["#text"] +
            " " +
            fmresult.name +
            "&access_token=" +
            geniuskey,
          onload: (response) => {
            if (JSON.parse(response.responseText).response.hits.length === 0)
              return;
            const geniusresult = JSON.parse(response.responseText).response
              .hits[0].result;
            document
              .getElementsByClassName(
                "HomeContentdesktop__CenteredFlexColumn-sc-1xfg7l1-1"
