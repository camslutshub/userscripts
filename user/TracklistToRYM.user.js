...
// @icon            https://raw.githubusercontent.com/TheLastZombie/userscripts/main/icons/TracklistToRYM.png
...
(async function () {
  ...
  if ((await GM.getValue("sites")) === undefined)
    await GM.setValue(
      "sites",
      sitestmp.map((x) => x.name)
    );
  if (
    (await GM.getValue("default")) === undefined ||
    !(await GM.getValue("sites")).includes(await GM.getValue("default"))
  )
    await GM.setValue("default", "Rate Your Music");
            ...
            switch (input.extractor) {
              case "json":
                JSON.parse(data).forEach((element) => {
                  ...
                  const index = input.index
                    ? reduceJson(element[input.parent], input.index)
                    : amount;
                  const title = input.title
                    ? reduceJson(element[input.parent], input.title)
                    : "";
                  const length = input.length
                    ? reduceJson(element[input.parent], input.length)
                    : "";
                  result += getResult(index, title, length);
                });
                artist = input.artist
                  ? reduceJson(JSON.parse(data)[input.parent], input.artist)
                  : "";
                album = input.album
                  ? reduceJson(JSON.parse(data)[input.parent], input.album)
                  : "";
                break;
              ...
              default:
                ...
                return printMessage(
                  "error",
                  input.extractor +
                    " is not a valid extractor. This is (probably) not your fault, please report this on <a href='https://github.com/TheLastZombie/userscripts/issues/new?title=" +
                    input.extractor +
                    "%20is%20not%20a%20valid%20extractor&labels=TracklistToRYM'>GitHub</a>."
                );
            }
